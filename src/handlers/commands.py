...
OWN_LINK_PROMPT_TEXT: str = (
    "🔗 Отправь свою ссылку на видео с рецептом\n"
    "\n"
    "1️⃣ Открой Shorts, Reels, TikTok или VK Клипы\n"
    "2️⃣ Нажми «Поделиться» → «Копировать ссылку»\n"
    "3️⃣ Вставь ссылку в этот чат одним сообщением\n"
    "\n"
    "Я сразу начну разбор — как с обычным сообщением от тебя."
)


def format_example_simulation_text(url: str) -> str:
    """Текст «как будто пользователь отправил ссылку» для демо-примера."""
    from html import escape

    safe_url = escape(url.strip())
    return (
        "📎 <b>Пример:</b> имитируем, что ты отправил ссылку\n"
        "\n"
        f"👤 <b>Ты:</b> {safe_url}"
    )
...
def format_tutorial_text(cfg: "Config") -> str:
    ...
    return (
        ...
        "\n"
        "Нажми «🔥 Протестировать пример» или «🔗 Отправить свою ссылку»!"
    )
